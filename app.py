...
# ── FocalLogRegSklearn 클래스 정의 (single pkl 로드에 필요) ────
...
st.caption("국민건강영양조사 7·8기 (2016–2021) · 50세 이상")
st.divider()
...
if household == "1인가구":
    model        = bundle_single['model']
    scaler       = bundle_single['scaler']
    feature_cols = bundle_single['feature_columns']
    scale_cols   = bundle_single['scale_cols']
    threshold    = bundle_single['threshold']       # 0.4816
else:
    model        = bundle_multi['model']            # Pipeline (스케일러 내장 없음)
    scaler       = None                             # multi pkl에 scaler 키 없음
    feature_cols = bundle_multi['feature_columns']
    scale_cols   = []                               # Pipeline이 자체 처리
    threshold    = bundle_multi['threshold']        # 0.5
...
with st.expander("① 인구사회학적 정보", expanded=True):
    ...
    with c2:
        ...
        occp = st.selectbox("직업", ["화이트칼라 (관리직·전문직)", "블루칼라 (농림·기능·단순노무)", "기타직종 (서비스·판매·무직 등)"])
    with c3:
...
with st.expander("④ 생활 습관", expanded=True):
    ...
    with c1:
        ...
        smoking_cvh = st.selectbox("흡연 상태", ["비흡연", "흡연"])
    with c2:
        BD1_11 = st.selectbox(
            "음주 빈도",
    ...
    with c3:
        mh_stress  = st.selectbox("스트레스 인지 수준", ["적게 느낌", "많이 느낌"])
        BO1_1      = st.selectbox("1년간 체중 변화", ["변화 없음", "체중 감소", "체중 증가"])
        ...
        be8_2 = st.number_input("좌식 시간 추가 (분)", min_value=0, max_value=59, value=0)
...
def encode_input():
    ...
    occp_val   = {"화이트칼라 (관리직·전문직)": "white",
                "블루칼라 (농림·기능·단순노무)": "blue",
                "기타직종 (서비스·판매·무직 등)": "other"}[occp]
    region_val = 1 if region == "수도권" else 0
    pa_val     = 1 if pa_aerobic == "충족" else 0
    smoke_val  = 1 if smoking_cvh == "흡연" else 0      # 이진 0/1
    bd1_val    = {"최근 1년간 전혀 마시지 않음": 0, "월 1회 미만": 2,
                "월 1회 정도": 3, "월 2~4회": 4,
                "주 2~3회": 5, "주 4회 이상": 6}[BD1_11]
    stress_val = 1 if mh_stress == "많이 느낌" else 0
    bo1_val    = {"변화 없음": 1.0, "체중 감소": 2.0, "체중 증가": 3.0}[BO1_1]
    ...
    # sex: feature_cols에 'sex' 있으면 그대로, 'sex_2.0' 방식이면 원핫
    if 'sex' in feature_cols:
        row['sex'] = sex_val
    elif 'sex_2.0' in feature_cols:
        row['sex_2.0'] = 1 if sex_val == 2.0 else 0
    ...
    # occp 원핫
    for o in ['white', 'other', 'blue']:
        key = f'occp_{o}'
        if key in feature_cols:
            row[key] = 1 if occp_val == o else 0

    df_input = pd.DataFrame([row])[feature_cols]

    # single: 외부 scaler 적용 / multi: Pipeline이 자체 처리하므로 스케일링 불필요
